...
$form_action = WATER_CERTIFICATES_PLUGIN_ADMIN_URL . '&action=add';

if ($mode == 'edit')
	$form_action = WATER_CERTIFICATES_PLUGIN_ADMIN_URL . '&action=edit';


update_option(
	WATER_CERTIFICATES_PLUGIN_NAME.'_media_selector_attachment_id',
...
<div class="wrap">
	<h1 class="wp-heading-inline"><?= $form_title ?> сертификат</h1>
	<a href="<?= WATER_CERTIFICATES_PLUGIN_ADMIN_URL ?>" class="page-title-action">← Назад</a>
	...
	<form method="post" action="<?= $form_action ?>" novalidate="novalidate">
		...
		<table class="form-table">
			<tbody>
				...
				<tr>
					<th scope="row">
						<label for="subtitle">Alt</label>
					</th>
					<td>
						<textarea name="data_alt" type="text" id="alt" value="<?= self::$model->subtitle ?>" class="regular-text"></textarea>
					</td>
				</tr>
				<tr>
					<th scope="row">
					...
					<td>
						<div class='image-preview-wrapper'>
							<img id='image-preview' src='<?php echo self::$model->get_image_attachment_filepath( get_option( WATER_CERTIFICATES_PLUGIN_NAME.'_media_selector_attachment_id' ) ); ?>' height='100'>
						</div>
						<input id="upload_image_button" type="button" class="button" value="Выбрать изображение" />
						<input type='hidden' name='data_image_attachment_id' id='image_attachment_id' value='<?php echo get_option( WATER_CERTIFICATES_PLUGIN_NAME.'_media_selector_attachment_id' ); ?>'>
					</td>	
				</tr>
			</tbody>
		</table>
