...
public function get($id){
	...
	$query = "SELECT * FROM `" . WATER_CERTIFICATES_DB_TABLE_NAME . "` WHERE id = '" . $id . "' LIMIT 1";
...
public function get_list(){
	...
	$query = "SELECT * FROM `" . WATER_CERTIFICATES_DB_TABLE_NAME . "`";
...
public function get_image_attachment_filepath($image_attachment_id){
	if (is_null($image_attachment_id) || empty($image_attachment_id) || ($image_attachment_id == 0))
		return plugins_url( WATER_CERTIFICATES_PLUGIN_NAME . '/static/images/no-avatar.png' ) ;
	else
...
public function delete($id){
	...
	return  $wpdb->delete(
		WATER_CERTIFICATES_DB_TABLE_NAME,
				[
					'id' => $id
				]
			);
